...
- Question: runtime error モデル
  ...
  - Decision:
    ...
    - procframe の構造化 error は `*Status` を保持する `StatusError` で表現する
...
- [x] Theme: Codegen + CLI unary end-to-end
  ...
  - Must Not Break: Theme 1 の public API (`Request`, `Response`, `ServerStream`, `Meta`, `Status/StatusError`, `Code`) の意味論
...
- [x] Theme: Help/Schema メタデータ伝播
  ...
  - Why not split vertically further?: 全メタデータ伝播は同一の codegen パス (field descriptor → flag registration + schema struct) を共有し、出力面も help text + schema JSON の2つだけ
